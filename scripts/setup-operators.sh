...
# Prometheus /metrics ports per operator. Layout: 92<op_idx><svc> where op_idx is
# zero-based and svc is 1=coordinator, 2=user-api, 3=block-indexer, 4=log-indexer.
# Keeps op_1 on the base.toml defaults (9201-9204) so single-operator runs stay
# backward-compatible.
operator_coordinator_metrics_port() {
  local op_num="$1"

  echo "$((9201 + (op_num - 1) * 10))"
}

operator_user_api_metrics_port() {
  local op_num="$1"

  echo "$((9202 + (op_num - 1) * 10))"
}

operator_block_indexer_metrics_port() {
  local op_num="$1"

  echo "$((9203 + (op_num - 1) * 10))"
}

operator_log_indexer_metrics_port() {
  local op_num="$1"

  echo "$((9204 + (op_num - 1) * 10))"
}
...
UB__COORDINATOR__BROKER__CLIENT_ID=$((100 + op_num))
...
# Prometheus scrape config spanning every prepared operator. Targets use the
# per-operator host ports published by docker/operator/docker-compose.yml; the
# `operator` label is what tells the operators' identically-named series apart
# (the in-process `service` label only distinguishes the four binaries).
write_prometheus_config() {
  local config_path="${BASE_STORAGE_PATH}/.union_bridge/prometheus.yml"
  local metrics_host="${PROMETHEUS_METRICS_HOST:-host.docker.internal}"
  local op_num

  mkdir -p "$(dirname "${config_path}")"

  {
    echo "# Generated by scripts/setup-operators.sh -- do not edit by hand."
    echo "global:"
    echo "  scrape_interval: 15s"
    echo ""
    echo "scrape_configs:"
    echo "  - job_name: union-operators"
    echo "    static_configs:"
    for op_num in "${OPERATORS_TO_RUN[@]}"; do
      echo "      - targets:"
      echo "          - ${metrics_host}:$(operator_coordinator_metrics_port "${op_num}")"
      echo "          - ${metrics_host}:$(operator_user_api_metrics_port "${op_num}")"
      echo "          - ${metrics_host}:$(operator_block_indexer_metrics_port "${op_num}")"
      echo "          - ${metrics_host}:$(operator_log_indexer_metrics_port "${op_num}")"
      echo "        labels: { operator: op_${op_num} }"
    done
  } > "${config_path}"

  echo "- Wrote Prometheus scrape config for ${#OPERATORS_TO_RUN[@]} operator(s) to ${config_path}"
  echo "  Run Prometheus against it with:"
  echo "    docker run --rm -p 9090:9090 -v \"${config_path}:/etc/prometheus/prometheus.yml\" prom/prometheus"
}
