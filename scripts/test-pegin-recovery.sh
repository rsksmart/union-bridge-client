...
# requests a pegin, restarts coordinators without wiping storage, and then waits
# for the correlated pegin completion marker and balance changes.
...
Runs a pegin recovery e2e:
  1. Optionally runs setup + committee.
  2. Requests a pegin.
  3. Waits briefly so processors can observe and persist in-flight state.
  4. Restarts coordinators without wiping storage.
  5. Waits for the correlated pegin completion marker and balance checks.
...
Options:
  ...
  --skip-prereqs     Do not run setup + committee; assume the stream is already ready.
...
Examples:
  ...
  bash scripts/test-pegin-recovery.sh --env local-anvil --skip-prereqs --settle-seconds 8
EOF
}
...
initialize_pegin_recovery() {
    ...
    log "Configuration: recovery=pegin, env=${SCRIPT_ENV}, ops=${NUM_OPERATORS}, stream=${STREAM_ID}, amount=${VALUE}, run_prereqs=${RUN_PREREQS}, settle_seconds=${RECOVERY_SETTLE_SECONDS}"
    echo ""
}

run_pegin_recovery_phase() {
    step "Pegin Recovery"
    ...
    prepare_user_wallet_for_pegin || return 1
    ...
    EXPECTED_PEGIN_TXID=$(run_user_pegin_and_capture_txid "$RSK_ADDRESS" "$VALUE") || return 1
    success "Pegin transaction created: ${EXPECTED_PEGIN_TXID}"
    echo ""
    ...
    assert_no_correlated_completion_marker "pegin" || return 1
    ...
    if ! wait_for_correlated_completion_markers "pegin" "$NUM_OPERATORS" "$max_blocks"; then
        warn "Pegin completion marker not found after recovery restart"
        return 1
    fi
    echo ""

    verify_user_rsk_balance_increase_for_pegin "$user_rsk_balance_before_wei" || return 1
    verify_user_btc_balance_decrease_for_value "$user_btc_balance_before_sat" || return 1

    local pegin_end_time
    pegin_end_time=$(date +%s)
    PEGIN_DURATION=$((pegin_end_time - pegin_start_time))
    success "Pegin recovery completed in $(format_duration "$PEGIN_DURATION")"
    echo ""
}

main() {
    initialize_pegin_recovery "$@" || return 1

    if [[ "$RUN_PREREQS" == true ]]; then
        run_setup_and_committee_phases || return 1
    fi

    run_pegin_recovery_phase || return 1

    step "Complete"
    success "Pegin recovery e2e completed successfully"
}
