...
usage() {
  echo "Usage: $0 [-f <block_finality>] [-b <block_height>] [-c <true|false>] [-e <env>] -t <tag>"
  echo ""
...
while getopts "f:b:c:e:t:" opt; do
  case "$opt" in
...
# Run log-indexer
CMD="RUST_BACKTRACE=1 cargo run --bin log-indexer -- -l $target_log_config_file -c $target_config_folder"
echo "Starting log-indexer with command: $CMD"
eval $CMD