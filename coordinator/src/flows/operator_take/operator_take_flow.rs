...
use common::msg_broker::bitvmx_types::{
    ...
    IncomingBitVMXApiMessages, OPERATOR_TAKE_TX, VariableTypes,
};
    ...
    BC: BitVmxBrokerClientApi,
{
    ...
    pub fn start_step(&mut self, next_step: Steps) -> Result<StepOutcome> {
    ...
    pub fn complete_step(&mut self, data: StepData) -> Result<StepOutcome> {
    ...
    fn enter_request_transaction_info(&mut self) -> Result<()> {
        ...
        let accept_pegin_pid =
            get_accept_pegin_pid(self.committee_id_uuid(), self.state.trigger_data.slot_index)?;
    ...
    pub fn is_waiting_comm_info(&self) -> bool {
    ...
    pub fn matches_accept_pegin_pid(&self, program_id: &Uuid) -> bool {
    ...
    #[cfg(test)]
    pub fn has_advance_funds_registered(&self) -> bool {
    ...
    pub fn matches_pegout(&self, pegout_id: Hash256) -> bool {
        self.state.trigger_data.pegout_id == pegout_id
    }

    /// True when this flow's trigger matches the given `(committee_id, slot_id)`.
    pub fn matches_committee_slot(&self, committee_id: u128, slot_id: u64) -> bool {
