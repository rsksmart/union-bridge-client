...
pub struct RejectPeginFlow<CG, BC, S>
where
    ...
    S: CoordinatorStoreApi,
{
    ...
    pub fn trigger(&self) -> &RejectPeginTrigger {
    ...
    pub fn get_reject_pegin_txid(&self) -> Option<Txid> {
        self.state.ctx.reject_pegin_tx_status.as_ref().map(|status| status.tx_id)
    }

    pub fn request_transaction_status(&self) -> Result<IncomingBitVMXApiMessages> {
        let tx_id =
