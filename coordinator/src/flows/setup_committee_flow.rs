    ...
    BC: BitVmxBrokerClientApi,
{
    ...
    fn request_dispute_aggregated_key(&mut self) -> Result<()> {
        ...
        self.send_bitvmx_msg(IncomingBitVMXApiMessages::GetAggregatedPubkey(dispute_key_id));
        Ok(())
    }
}
    ...
    fn complete_step_and_next(&mut self, req_id: Option<Uuid>, data: StepData) -> Result<()> {
        ...
        match current_step {
            ...
            Steps::SetupTakeAggregatedKey => {
                let pubkey = data.into_pubkey()?;
                self.state.ctx.agg_take_key = Some((req_id.unwrap(), Some(pubkey)));
            }
            ...
            Steps::SetupDisputeAggregatedKey => {
                let pubkey = data.into_pubkey()?;
                self.state.ctx.agg_dispute_key = Some((req_id.unwrap(), Some(pubkey)));
            }
        ...
        match event {
            ...
            OutgoingBitVMXApiMessages::AggregatedPubkey(req_id, pubkey) => {
                ...
                if let Some(flow) = self.get_flow_for_request_id(req_id) {
                    ...
                    bail!("No flow found for OutgoingBitVMXApiMessages::AggregatedPubkey and id {req_id}");
                }
            }
            OutgoingBitVMXApiMessages::AggregatedPubkeyNotReady(req_id) => {
                ...
                if let Some(flow) = self.get_flow_for_request_id(req_id) {
                    ...
                    bail!("No flow found for OutgoingBitVMXApiMessages::AggregatedPubkeyNotReady and id {req_id}");
                }
            }
            _ => {}
        }
