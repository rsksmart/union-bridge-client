...
impl<P: RskProvider, S: BlockStore> BlockIndexer<P, S> {
    ...
    fn init_db_if_required(&self) -> Result<()> {
        ...
        let initial_block_node = self
            .rsk_provider
            .get_block_by_hash(self.initial_block_hash.clone())
    ...
    fn listen_blocks(
        ...
        while self.is_running() {
            ...
            let extends_canonical = new_block.parent_block_hash() == local_best_block.block_hash();
    ...
    fn backward_sync(&self, starting_block: &RskBlock) -> Result<()> {
        ...
        loop {
            ...
            let is_reorg = store_block.map_or(false, |sb| sb.block_hash() != new_block.block_hash());
            ...
            if self.initial_block_hash == new_block.block_hash() || new_block.number() == 0 {
    ...
    fn resume_pending_backward_sync(&self) -> Result<()> {
            ...
            .context("Resuming Pending Backward Sync")?
        {
            match self
                .rsk_provider
                .get_block_by_hash(checkpoint.parent_block_hash())
    ...
    fn full_sync_backward_syncs(&self) -> Result<()> {
        ...
        for i in 1..max_attempts {
            ...
            if let Some(store_best_block) = self
                ...
                .context("On Full Sync Backward Sync rounds")?
            {
                let is_full_sync = provider_best_block.block_hash() == store_best_block.block_hash();
                if is_full_sync {
