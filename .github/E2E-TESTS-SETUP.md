...
3. **union_bridge_e2e_framework:** Secrets `TOKEN_CONTRACTS`, `TOKEN_FAIRGATE` (token for private dependency repos), `USER_BITCOIN_WIF`, `MEMBER_BITCOIN_WIF`. Optional: `GHCR_USERNAME` if PAT owner differs from `github.actor`.
