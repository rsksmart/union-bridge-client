...
1. **union-bridge-client:** Secret `E2E_FRAMEWORK_GITHUB_TOKEN` (PAT with `repo` + `workflow`, access to `union_bridge_e2e_framework`). Needed to trigger the e2e workflow (else 404). Configure in repository settings.
2. **union-bridge-client:** Branch protection for `main` → require status check `e2e-smoke-tests` (optional until enforced).
3. **union_bridge_e2e_framework:** Secrets `TOKEN_CONTRACTS`, `TOKEN_FAIRGATE` (PAT for private BitVMX-related git dependencies—the name is historical), `USER_BITCOIN_WIF`, `MEMBER_BITCOIN_WIF`. Optional: `GHCR_USERNAME` if PAT owner differs from `github.actor`.
