...
# Script to run the happy path until pegin completion
#
# prerequisites:
...
#
# usage: bash tests/run-until-pegin.sh [--env <local|local-docker>]
...
cleanup() {
    ...
    rm -f /tmp/apply-operators-$$ /tmp/pegin-$$
}
...
if ! bash cli-operations.sh user pegin -a $RSK_ADDRESS -v $VALUE -p $PACKET_NUMBER --env "$SCRIPT_ENV" --execute > /tmp/pegin-$$ 2>&1; then
    warn "Command failed! Output:"
    cat /tmp/pegin-$$
    rm -f /tmp/pegin-$$
    exit 1
...
# step 4: verify pegin completion
step "Step 4: Verify Pegin Completion"

# search for PeginFlow completion with recent timestamp
# pattern: "PeginFlow <uuid>: Done"
# allow 5 minutes margin (300 seconds) before pegin start for clock differences
TIME_MARGIN=300
MIN_TIME=$((PEGIN_START_TIME - TIME_MARGIN))

# Pattern for PeginFlow completion
PEGIN_PATTERN="PeginFlow [0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}: Done"

MATCHING_LINE=""
MATCHING_SOURCE=""

if [[ "$SCRIPT_ENV" == "local-docker" ]]; then
    log "Checking Docker logs from all operators for PeginFlow completion..."
    echo ""

    for op_id in 1 2 3 4; do
        project="op_${op_id}"
        found_line=$(find_recent_log_match "$PEGIN_PATTERN" "$project" "$MIN_TIME" "docker")
    ...
    log "Checking local logs from all operators for PeginFlow completion..."
    echo ""
    ...
    for log_file in logs/coordinator-*.log; do
        ...
        found_line=$(find_recent_log_match "$PEGIN_PATTERN" "$log_file" "$MIN_TIME" "file")
        if [ -n "$found_line" ]; then
            MATCHING_LINE="$found_line"
...
if [ -n "$MATCHING_LINE" ]; then
    ...
    warn "PeginFlow completion not detected in any operator logs"
    if [[ "$SCRIPT_ENV" == "local-docker" ]]; then
...
if [ "$SUCCESS" = true ]; then
    success "E2E test completed successfully - Pegin completed!"
else
    warn "E2E test completed with warnings - PeginFlow completion not verified"
    exit 1
fi
