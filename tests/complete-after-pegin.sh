...
mine_bitcoin() {
    # First, mine 15 blocks with legacy_wallet
    local legacy_wallet_arg="-rpcwallet=legacy_wallet"
    local legacy_address=$(bitcoin-cli -regtest -rpcuser=foo -rpcpassword=rpcpassword $legacy_wallet_arg getnewaddress 2>/dev/null)
    if [ -z "$legacy_address" ]; then
        echo -e "${YELLOW}[WARN]${NC} Failed to get Bitcoin address for legacy_wallet" >&2
        return 1
    fi
    
    log "Mining 15 blocks with legacy_wallet..."
    for i in {1..15}; do
        bitcoin-cli -regtest -rpcuser=foo -rpcpassword=rpcpassword $legacy_wallet_arg generatetoaddress 1 "$legacy_address" &>/dev/null || true
        sleep 5
    done
    success "Mined 15 blocks with legacy_wallet"
    
    # Then, mine 15 blocks with mainwallet
    local main_wallet_arg="-rpcwallet=mainwallet"
    local main_address=$(bitcoin-cli -regtest -rpcuser=foo -rpcpassword=rpcpassword $main_wallet_arg getnewaddress 2>/dev/null)
    if [ -z "$main_address" ]; then
        echo -e "${YELLOW}[WARN]${NC} Failed to get Bitcoin address for mainwallet" >&2
        return 1
    fi
    
    log "Mining 15 blocks with mainwallet..."
    for i in {1..15}; do
        bitcoin-cli -regtest -rpcuser=foo -rpcpassword=rpcpassword $main_wallet_arg generatetoaddress 1 "$main_address" &>/dev/null || true
        sleep 5
    done
    success "Mined 15 blocks with mainwallet"
    
    # Continue mining indefinitely with mainwallet
    log "Continuing to mine indefinitely with mainwallet..."
    while true; do
        bitcoin-cli -regtest -rpcuser=foo -rpcpassword=rpcpassword $main_wallet_arg generatetoaddress 1 "$main_address" &>/dev/null || true
        sleep 5
    done
