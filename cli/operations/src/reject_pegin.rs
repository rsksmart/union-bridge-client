...
pub(crate) async fn request_reject_pegin(
    ...
    let endpoint = resolve_member_endpoint(environment.clone(), operator_id)?;
...
fn resolve_member_endpoint(environment: Environment, operator_id: Option<u8>) -> Result<String> {
    let selected_operator_id = if environment.is_remote() {
        operator_id.ok_or_else(|| {
            anyhow!("--operator-id is required when using --env {}", environment.get_name())
        })?
    } else {
        operator_id.unwrap_or(1)
    };
...
mod tests {
    ...
    fn local_defaults_to_operator_one() {
        let endpoint =
            resolve_member_endpoint(Environment::LocalAnvil, None).expect("local should default");
        assert_eq!(endpoint, "localhost:40001");
    }

    #[test]
    fn remote_requires_operator_id() {
        let err = resolve_member_endpoint(Environment::Remote("alphanet".to_string()), None)
