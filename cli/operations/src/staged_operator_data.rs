...
fn collect_local_rsk_addresses(role: RskRole, first_only: bool) -> Result<Vec<(String, String)>> {
    ...
    for operator_id in selected_local_operator_ids(first_only) {
        let password = resolve_local_env_value(&storage_root, operator_id, "KEY_STORE_PASSWORD")?;
...
fn collect_remote_rsk_addresses(
    ...
    for (operator_id, host) in hosts {
        ...
        let password = resolve_remote_env_value(&target, operator_id, "KEY_STORE_PASSWORD")?;
...
fn remote_keystore_path(operator_id: u8, role: RskRole) -> String {
    format!(
        "~/{UNION_BRIDGE_DIR}/op_{operator_id}/union-client/keystore/{}",
        role.keystore_name()
    )
}
...
fn discover_remote_operator_id(target: &str) -> Result<u8> {
    let listing = run_ssh_capture(
        target,
        &format!("for dir in ~/{UNION_BRIDGE_DIR}/op_*; do [ -d \"$dir\" ] && basename \"$dir\"; done"),
...
mod tests {
    ...
    #[test]
    fn ignores_non_assignment_lines_before_matching_env_value() {
        let value = lookup_key_in_env_contents(
            "set -a\nexport KEY_STORE_PASSWORD=\"secret\"\n",
            "KEY_STORE_PASSWORD",
        );
    ...
    #[test]
    fn parses_single_remote_operator_id_from_listing() {
        assert_eq!(parse_remote_operator_ids("op_3\n"), vec![3]);
    }

    #[test]
    fn deduplicates_remote_operator_ids_from_listing() {
        assert_eq!(parse_remote_operator_ids("op_2\nop_2\n"), vec![2]);
    }
