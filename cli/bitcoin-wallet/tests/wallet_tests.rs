...
fn create_transaction_does_not_consume_utxo_and_creates_change_until_broadcasted() {
    ...
    wallet.import_private_key(&wallet_wif).expect("import private key");

    set_fake_rpc_client(&mut wallet);

    let input_txid = Txid::from_slice(&[0x11; 32]).expect("txid");
    let utxo_amount = 50_000;
    wallet.register_utxo(OutPoint::new(input_txid, 0), utxo_amount).expect("register utxo");

    let target_script = recipient_script();

    let tx_amount = 20_000;
    let txs =
        wallet.create_transactions(vec![target_script.clone()], 20_000, 1).expect("create tx");

    assert_eq!(txs.len(), 1);
    let created = &txs[0];

    assert_eq!(created.transaction.output.len(), 2);
    assert_eq!(created.transaction.output[0].value, Amount::from_sat(tx_amount));
    ...
    assert_eq!(created.fee_sat, expected_fee, "fee should match rate * vsize");
...
fn dust_change_is_added_to_fee() {
    ...
    let wallet_key = PrivateKey::new(wallet_secret, Network::Regtest);
    wallet.import_private_key(&wallet_key.to_wif()).expect("import private key");

    let input_txid = Txid::from_slice(&[0x21; 32]).expect("txid");
    let input_value = 50_000_u64;
    wallet.register_utxo(OutPoint::new(input_txid, 0), input_value).expect("register utxo");

    let target_script = recipient_script();
    let send_value = 49_800_u64;

    let txs = wallet.create_transactions(vec![target_script], send_value, 1).expect("create tx");
...
fn switching_addresses_reloads_corresponding_utxos() {
    ...
    let addr_one = wallet.import_private_key(&key_one.to_wif()).expect("import first key");

    let txid_one = Txid::from_slice(&[0x33; 32]).expect("txid one");
    wallet.register_utxo(OutPoint::new(txid_one, 0), 42_000).expect("register first utxo");
    assert_eq!(wallet.utxos().len(), 1, "first address should have one utxo");
    ...
    let addr_two = wallet.import_private_key(&key_two.to_wif()).expect("import second key");

    assert!(wallet.utxos().is_empty(), "newly active second address should start without utxos");

    let txid_two = Txid::from_slice(&[0x44; 32]).expect("txid two");
    wallet.register_utxo(OutPoint::new(txid_two, 1), 55_000).expect("register second utxo");
    assert_eq!(wallet.utxos().len(), 1, "second address should have one utxo");

    wallet.switch_active_address(addr_one.clone()).expect("switch back to first address");
    assert_eq!(wallet.utxos().len(), 1, "first address utxo set restored");
    assert_eq!(wallet.utxos()[0].outpoint.txid, txid_one);

    wallet.switch_active_address(addr_two.clone()).expect("switch to second address");
...
fn switching_networks_with_mismatched_keys_succeeds() {
    ...
    wallet.import_private_key(&key.to_wif()).expect("import regtest key");

    let changed = wallet.set_network(Network::Bitcoin).expect("switch to bitcoin");
...
fn utxos_with_timestamps_all_lists_active_first() {
    ...
    wallet.register_utxo(OutPoint::new(txid_one, 0), 21_000).expect("register first utxo");
    ...
    wallet.register_utxo(OutPoint::new(txid_two, 1), 34_000).expect("register second utxo");

    let entries = wallet.utxos_with_timestamps_all().expect("collect utxos across addresses");

    assert_eq!(entries.len(), 2, "both addresses should be reported");
    assert_eq!(entries[0].0, addr_two, "active address must appear first");
    assert_eq!(entries[0].1.len(), 1, "active address should list its utxos");
    assert_eq!(entries[0].1[0].0.outpoint, OutPoint::new(txid_two, 1));
    assert_eq!(entries[0].1[0].0.value_sat, 34_000);

    assert_eq!(entries[1].0, addr_one, "non-active address listed after active");
    ...
    let timestamps_are_nonzero =
        entries.iter().flat_map(|(_, utxos)| utxos.iter().map(|(_, ts)| *ts)).all(|ts| ts > 0);
...
fn utxos_with_timestamps_all_includes_store_only_addresses() {
    ...
    let address_string = {
        ...
        let key = PrivateKey::new(secret, Network::Regtest);
        let address = wallet.import_private_key(&key.to_wif()).expect("import key");

        let txid = Txid::from_slice(&[0x77; 32]).expect("txid");
        wallet.register_utxo(OutPoint::new(txid, 0), 45_000).expect("register utxo");
    ...
    let entries = wallet.utxos_with_timestamps_all().expect("load utxos across addresses");
...
fn utxo_listings_are_sorted_by_timestamp() {
    ...
    let mut wallet = Wallet::new(db_root.clone(), WalletMode::User).expect("wallet");
    wallet.import_private_key(&wif).expect("reimport key for listing");
    ...
    let aggregated_reloaded =
        wallet_reloaded.utxos_with_timestamps_all().expect("aggregate utxos after reload");
...
impl FakeTransport {
    fn new() -> Self {
        Self { last_method: Arc::new(Mutex::new(None)) }
    }
}
...
impl jsonrpc::Transport for FakeTransport {
    fn send_request(&self, req: Request) -> Result<Response, Error> {
        ...
        *self.last_method.lock().expect("lock last_method") = Some(method.clone());
        ...
        Ok(Response { result, error: None, id: req.id, jsonrpc: Some("2.0".to_string()) })
    }
