...
fn utxos_persist_across_wallet_instances() {
    ...
    // First wallet registers a UTXO.
    {
        ...
        wallet.import_private_key(&key.to_wif()).expect("import key");

        let txid = Txid::from_slice(&[0x22; 32]).expect("txid");
        wallet.register_utxo(OutPoint::new(txid, 1), 75_000).expect("register utxo");
    }
    ...
    let secret = secp256k1::SecretKey::from_slice(&[42u8; 32]).expect("secret");
    let key = PrivateKey::new(secret, Network::Regtest);
    wallet.import_private_key(&key.to_wif()).expect("re-import key");
...
fn load_by_address_returns_only_matching_entries() {
    ...
    wallet.register_utxo(OutPoint::new(txid_one, 0), 11_000).expect("register first utxo");
    ...
    wallet.register_utxo(OutPoint::new(txid_two, 1), 22_000).expect("register second utxo");
    ...
    let entries_one = store.load_by_address(&addr_one).expect("load entries for first address");
    assert_eq!(entries_one.len(), 1);
    assert_eq!(entries_one[0].0.txid, txid_one);
    assert_eq!(entries_one[0].1.value_sat, 11_000);
    assert_eq!(entries_one[0].1.address.as_deref(), Some(addr_one_str.as_str()));

    let addr_two_str = addr_two.to_string();
    let entries_two = store.load_by_address(&addr_two).expect("load entries for second address");
    assert_eq!(entries_two.len(), 1);
    assert_eq!(entries_two[0].0.txid, txid_two);
    assert_eq!(entries_two[0].1.value_sat, 22_000);
    assert_eq!(entries_two[0].1.address.as_deref(), Some(addr_two_str.as_str()));
...
fn switching_networks_preserves_utxos_per_network() {
    ...
    wallet.register_utxo(OutPoint::new(txid, 0), 33_000).expect("register utxo");

    wallet.set_network(Network::Testnet).expect("switch to testnet");

    wallet.set_network(Network::Regtest).expect("switch back to regtest");

    let reloaded_addr = wallet.import_private_key(&key.to_wif()).expect("re-import key");
