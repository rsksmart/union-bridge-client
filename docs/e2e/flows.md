...
```mermaid
%%{init: {'sequence': {'diagramMarginY': 24, 'messageMargin': 42, 'boxMargin': 20, 'noteMargin': 14, 'actorMargin': 80}}}%%
sequenceDiagram
    ...
    Note over UC: Init — complete_step(UserRequest) → start ValidateBalances
...
sequenceDiagram
    ...
    Note over UC,BV: Until PeginRequested confirms, the client tracks this flow by Bitcoin txid
    Note over UC,BV: Afterward BitVMX uses a stable flow id from committee + slot
...
sequenceDiagram
    ...
    Note over UC,BV: flow_id is committee+slot from flow creation
    Note over UC,BV: BitVMX always uses this id for this pegout
    ...
    Note over UC,SG: BTC signature subflow — PG AllNoncesReady / AllSignaturesReady + BitVMX
    Note over UC,SG: Nonces and signatures on SignatureManager
    Note over UC: If advance_funds_timeout_secs elapses in DispatchTransaction without signatures
    Note over UC: trigger_operator_take — pegout flow Done (no USER_TAKE_TX, register_pegout, or PEG_OUT_COMPLETED)
    Note over UC: See Flow: Operator take
...
sequenceDiagram
    ...
    Note over UC: Pegout flow in DispatchTransaction — signature subflow active
    Note over UC: advance_funds_timeout_secs from pegout config
...
sequenceDiagram
    ...
    Note over UC,BV: adv_flow_id = advance-funds program id (hash from committee + slot)
    Note over UC,BV: committee_uuid = committee id for operator pubkey only
