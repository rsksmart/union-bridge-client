...
| `WALLET_RPC_URL`, `WALLET_RPC_USER`, `WALLET_RPC_PASSWORD` | shell or `.envrc` | the bitcoin-wallet CLI (e.g. `./scripts/test-flows.sh` funding via `mine_utxo`) | optional; the wallet CLI's own bitcoind RPC endpoint/creds (defaults to `127.0.0.1:18443` + `foo`/`rpcpassword` from `cli/bitcoin-wallet/config/regtest.toml`). Leave unset unless your BYO node's creds/host differ from those defaults; **unset them again when switching back to a default-creds node** (else `mine_utxo` hits the wrong creds and fails with `HTTP 401`) |
